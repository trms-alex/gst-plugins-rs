            ...
            .and_then(|results| results.drain(..).next())
        {
            result
                ...
                .map(|mut items| {
                    ...
                    gst::trace!(CAT, imp = self, "draining items from index {}", state.partial_index);
                    ...
                    for item in items.drain(state.partial_index..) {
                        ...
                        if let Some(content) = item.content {
                            ...
                            while let Some((discont_rtime, pts)) = state.disconts.pop_front() {
                                gst::trace!(CAT, imp = self, "checking if next discont is reached: {discont_rtime} {pts}");
                                ...
                                if aws_start_time >= discont_rtime {
                                    ...
                                    gst::info!(CAT, imp = self,
                                        "next discont is reached: \
                                         ...
                                         discont accumulator is now {}", state.discont_accumulator);
                                } else {
                            ...
                            if let Some(position) = state.in_segment.as_ref().unwrap().position() {
                                if pts > position {
                                    gst::log!(CAT, imp = self, "position lagging, queuing gap \
                                        with pts {position} and duration {}", pts - position);
                            ...
                            gst::log!(CAT, imp = self, "queuing item {content} with pts {pts} \
                                and duration {duration}");

                            state.in_segment.as_mut().unwrap().set_position(Some(pts + duration));
                        ...
                        gst::trace!(CAT, imp = self, "partial index is now {}", state.partial_index);
                    }
