...
impl Translate {
    ...
    async fn send(&self, to_translate: InputItems) -> Result<Vec<TranslateOutput>, Error> {
        ...
        let segment = state.segment.as_ref().unwrap().clone();

        let input_lang = settings.input_language_code.clone();
        let output_lang = settings.output_language_code.clone();
        let latency = settings.latency;
        let tokenization_method = settings.tokenization_method;
